...
import android.view.WindowSessionLocal;
...
public final class RotationPolicy {
    ...
    public static void setRotationLock(Context context, final boolean enabled) {
        ...
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                WindowSessionLocal wm = WindowSessionLocal.getInstance();
                if (enabled) {
                    wm.freezeRotation(-1);
                } else {
                    wm.thawRotation();
    ...
    public static void setRotationLockForAccessibility(Context context, final boolean enabled) {
        ...
        AsyncTask.execute(new Runnable() {
            @Override
            public void run() {
                WindowSessionLocal ws = WindowSessionLocal.getInstance();
                if (enabled) {
                    ws.freezeRotation(Surface.ROTATION_0);
                } else {
                    ws.thawRotation();
                }
            }
