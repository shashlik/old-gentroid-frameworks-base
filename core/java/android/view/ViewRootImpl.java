...
public final class ViewRootImpl implements ViewParent,
    ...
    final WindowSessionLocal mWindowSession;
    ...
    public ViewRootImpl(Context context, Display display) {
        mContext = context;
        mWindowSession = WindowSessionLocal.getInstance();
        mDisplay = display;
    ...
    static boolean isInTouchMode() {
        WindowSessionLocal windowSession = WindowManagerLocal.peekWindowSession();
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView) {
        synchronized (this) {
            if (mView == null) {
                ...
                if (res < WindowManagerLocal.ADD_OKAY) {
                    ...
                    setAccessibilityFocus(null, null);
                    switch (res) {
                        case WindowManagerLocal.ADD_BAD_APP_TOKEN:
                        case WindowManagerLocal.ADD_BAD_SUBWINDOW_TOKEN:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window -- token " + attrs.token
                                + " is not valid; is your activity running?");
                        case WindowManagerLocal.ADD_NOT_APP_TOKEN:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window -- token " + attrs.token
                                + " is not for an application");
                        case WindowManagerLocal.ADD_APP_EXITING:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window -- app for token " + attrs.token
                                + " is exiting");
                        case WindowManagerLocal.ADD_DUPLICATE_ADD:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window -- window " + mWindow
                                + " has already been added");
                        case WindowManagerLocal.ADD_STARTING_NOT_NEEDED:
                            // Silently ignore -- we would have just removed it
                            // right away, anyway.
                            return;
                        case WindowManagerLocal.ADD_MULTIPLE_SINGLETON:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window " + mWindow +
                                " -- another window of this type already exists");
                        case WindowManagerLocal.ADD_PERMISSION_DENIED:
                            throw new WindowManager.BadTokenException(
                                "Unable to add window " + mWindow +
                                " -- permission denied for this window type");
                        case WindowManagerLocal.ADD_INVALID_DISPLAY:
                ...
                mAddedTouchMode = (res & WindowManagerLocal.ADD_FLAG_IN_TOUCH_MODE) != 0;
                mAppVisible = (res & WindowManagerLocal.ADD_FLAG_APP_VISIBLE) != 0;
    ...
    private void performTraversals() {
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            try {
                ...
                if (!mDrawDuringWindowsAnimating &&
                        (relayoutResult & WindowManagerLocal.RELAYOUT_RES_ANIMATING) != 0) {
            ...
            if (!mStopped) {
                boolean focusChangedDueToTouchMode = ensureTouchModeLocally(
                        (relayoutResult&WindowManagerLocal.RELAYOUT_RES_IN_TOUCH_MODE) != 0);
        ...
        if (mFirst) {
            ...
            if ((relayoutResult & WindowManagerLocal.RELAYOUT_RES_ANIMATING) != 0) {
        ...
        if ((relayoutResult & WindowManagerLocal.RELAYOUT_RES_FIRST_TIME) != 0) {
    ...
    private final static int MSG_DISPATCH_STANDALONE_REQ = 26;
    ...
    final class ViewRootHandler extends Handler {
        ...
        public String getMessageName(Message message) {
            switch (message.what) {
                ...
                case MSG_DISPATCH_STANDALONE_REQ:
                    return "MSG_DISPATCH_STANDALONE_REQ";
            }
    ...
    private int relayoutWindow(WindowManager.LayoutParams params, int viewVisibility,
        ...
        int relayoutResult = mWindowSession.relayout(
                ...
                viewVisibility, insetsPending ? WindowManagerLocal.RELAYOUT_INSETS_PENDING : 0,
    ...
    void doDie() {
                    ...
                    if (mWindowAttributesChanged || viewVisibilityChanged) {
                        ...
                        try {
                            if ((relayoutWindow(mWindowAttributes, viewVisibility, false)
                                    & WindowManagerLocal.RELAYOUT_RES_FIRST_TIME) != 0) {
                                mWindowSession.finishDrawing(mWindow);
                            }
                        } catch (RemoteException e) {
                        }
        ...
        WindowManagerLocal.getInstance().doRemoveView(this);
    }
    ...
    public void dispatchStandaloneReq() {
        Message msg = mHandler.obtainMessage(MSG_DISPATCH_STANDALONE_REQ);
        msg.setAsynchronous(true);
        mHandler.sendMessage(msg);
    }
    ...
    static class W extends IWindow.Stub {
        ...
        private final WindowSessionLocal mWindowSession;
