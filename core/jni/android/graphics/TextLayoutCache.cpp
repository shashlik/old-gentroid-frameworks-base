...
void TextLayoutCache::init() {
    ...
    if (mDebugEnabled) {
        ALOGD("Initialization is done - Start time = %ld", mCacheStartTime);
    }
...
static void logGlyphs(hb_buffer_t* buffer) {
    ...
    for (size_t i = 0; i < numGlyphs; i++) {
        ALOGD("         -- glyph[%zu] = %u, cluster = %u, advance = %0.2f, offset.x = %0.2f, offset.y = %0.2f", i,
                info[i].codepoint,
                info[i].cluster,
...
size_t TextLayoutShaper::shapeFontRun(const SkPaint* paint) {
    ...
    if (isComplexScript(hb_buffer_get_script(mBuffer))) {
        ...
        baseGlyphCount = 0; //paint->getBaseGlyphCount(firstUnichar);
    }
