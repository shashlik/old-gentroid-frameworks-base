...
public:
    ...
    virtual bool allocPixelRef(SkBitmap* bitmap, SkColorTable* ctable) {
        ...
        const int bytesPerPixel = bitmap->bytesPerPixel();
...
public:
    ...
    virtual bool allocPixelRef(SkBitmap* bitmap, SkColorTable* ctable) {
        if (bitmap->getSize() > mSize) {
            ALOGW("bitmap marked for reuse (%d bytes) can't fit new bitmap (%zu bytes)",
                    mSize, bitmap->getSize());
            return false;
        }
		const SkImageInfo& bitmapInfo = bitmap->info();
...
static jobject doDecode(JNIEnv* env, SkStreamRewindable* stream, jobject padding,
    ...
    if (!decoder->decode(stream, &decodingBitmap, SkBitmapConfigToColorType(prefConfig), decodeMode)) {
    ...
    if (willScale) {
        ...
        SkImageInfo imgInfo = SkImageInfo::Make(scaledWidth, scaledHeight, colorType, decodingBitmap.alphaType());
        outputBitmap->setInfo(imgInfo);
