...
public:
	...
	virtual void clipRRect(const SkRRect& path, SkRegion::Op op, bool antialias) {
		m_dstCanvas->clipRRect(path, op, antialias);	
	}
...
public:
    ...
    static void copyCanvasState(JNIEnv* env, jobject clazz,
        ...
        if (srcCanvas && dstCanvas) {
            ...
            if (kUnknown_SkColorType != srcCanvas->imageInfo().colorType() && kUnknown_SkColorType != dstCanvas->imageInfo().colorType()) {
    ...
    static jboolean isOpaque(JNIEnv* env, jobject jcanvas) {
        ...
        bool result = canvas->imageInfo().isOpaque();
    ...
    static jboolean clipRect_IIII(JNIEnv* env, jobject jcanvas, jint left,
        ...
        GraphicsJNI::getNativeCanvas(env, jcanvas)->clipRect(r);
        return JNI_TRUE;
    }
                                ...
                                jboolean hasAlpha, jlong paintHandle)
    {
        ...
        SkImageInfo imgInfo = SkImageInfo::Make(width, height, hasAlpha ? kRGBA_8888_SkColorType : kRGB_565_SkColorType, hasAlpha ? kOpaque_SkAlphaType : kIgnore_SkAlphaType);
        SkBitmap    bitmap;
        bitmap.setInfo(imgInfo);
