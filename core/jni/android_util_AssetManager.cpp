        ...
        case 0: // child
            {
                ...
                std::string root = GetAndroidRoot() + "/";
                std::string data = GetAndroidData() + "/";

				ALOGV("exec idmap with args: %s %s %s %s %s", "--scan",
                        (root + AssetManager::kOverlay).c_str(), "android",
                        (root + AssetManager::kSystemAssets).c_str(), (data + AssetManager::kResourceCache).c_str());
                execlp("idmap", "idmap", "--scan",
                        (root + AssetManager::kOverlay).c_str(), "android",
                        (root + AssetManager::kSystemAssets).c_str(), (data + AssetManager::kResourceCache).c_str(), (char*)NULL);
